"""
Idempotent importer.  Place at:
    <app>/management/commands/import_watch_parties.py

Usage:
    python manage.py import_watch_parties data.json
    python manage.py import_watch_parties data.json --skip-materialize
...
so a corrected export overwrites the prior load instead of duplicating it.
"""
...
from ...extraction_schema import ImportBundle  # adjust relative path to your layout
from ...models import (
    Team, Match, Venue, VenueAffiliation, Screening, ScreeningPolicy,
)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **opts):
        try:
            raw = json.loads(open(opts["path"], encoding="utf-8").read())
    ...
    def _load_teams(self, bundle) -> dict[str, Team]:
        ...
        for t in bundle.teams:
            obj, _ = Team.objects.update_or_create(
                ...
                defaults=dict(name=t.name, flag_emoji=t.flag_emoji, fifa_rank=t.fifa_rank,
    ...
    def _load_matches(self, bundle, teams) -> dict[int, Match]:
        ...
        for m in bundle.matches:
            obj, _ = Match.objects.update_or_create(
                ...
                defaults=dict(
                    stage=m.stage.value, group=m.group, kickoff=m.kickoff,
                    host_city=m.host_city, host_stadium=m.host_stadium,
    ...
    def _load_venues(self, bundle, teams) -> dict[str, Venue]:
        ...
        for v in bundle.venues:
            obj, _ = Venue.objects.update_or_create(
                slug=v.slug,
                defaults=dict(
                    name=v.name, venue_type=v.venue_type.value, environment=v.environment.value,
                    address=v.address, city=v.city, region=v.region.value,
                    latitude=v.latitude, longitude=v.longitude,
                    serves_alcohol=v.serves_alcohol, default_min_age=v.default_min_age,
                    evening_min_age=v.evening_min_age, evening_cutoff=v.evening_cutoff,
                    capacity=v.capacity, has_food=v.has_food, website=v.website,
                    source=v.source, source_url=v.source_url,
                    needs_review=v.needs_review, notes=v.notes,
                ),
            )
            ...
            for a in v.affiliations:
                VenueAffiliation.objects.create(
                    venue=obj, affiliation_type=a.affiliation_type.value,
                    team=teams.get(a.team_code) if a.team_code else None,
                    club=a.club, valid_from=a.valid_from, valid_to=a.valid_to, note=a.note,
                )
            out[v.slug] = obj
        return out
    ...
    def _load_screenings(self, bundle, venues, matches):
        for s in bundle.screenings:
            ...
            if not venue or not match:
                self.stderr.write(f"  skip screening: unknown venue/match "
                                  f"({s.venue_slug} / {s.match_number})")
                continue
            Screening.objects.update_or_create(
                venue=venue, match=match, starts_at=s.starts_at,
                defaults=dict(
                    ends_at=s.ends_at, cost_type=s.cost_type.value,
                    registration_required=s.registration_required,
                    entry_guaranteed=s.entry_guaranteed, price_note=s.price_note,
                    age_override=s.age_override, is_generated=False,
                    source=s.source, source_url=s.source_url,
                    needs_review=s.needs_review, notes=s.notes,
                ),
            )
        ...
        for v in bundle.venues:
            ...
            for p in v.policies:
                policy = ScreeningPolicy.objects.create(
                    venue=venue, policy_type=p.policy_type.value,
                    default_cost_type=p.default_cost_type.value, source=v.source,
                )
                if p.team_codes:
